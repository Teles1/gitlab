...
export default async (pluginConfig, context) => {
  ...
  const caCert = filesystem.existsSync(caCertPath) ? filesystem.readFileSync(caCertPath) : undefined;
  const apiOptions = { headers: { "PRIVATE-TOKEN": gitlabToken }, https: { certificateAuthority: caCert, rejectUnauthorized: false } };
        ...
        if (_url) {
          ...
          if (target === "generic_package") {
            ...
            uploadEndpoint = urlJoin(
              gitlabApiUrl,
              `/projects/${encodedRepoId}/packages/generic/release/${encodedVersion}/${encodedLabel}?${status ? `status=${status}&` : ""
              }select=package_file`
            );
